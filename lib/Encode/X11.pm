...
Compound text is found in window properties of C<COMPOUND_TEXT> type.  It's
...
variations for which charsets to use.  For now it prefers the original ICCCM
charsets latin-N, jis, ksc and gb for the benefit of older X clients, then
...
libX11 1.4.0) but have that HanExtra available if you think you might
encounter them.
...
Perl wide chars.  If drawing with the core X requests then just crack it
into segments of the various charsets and for each find a font of that
encoding.  Some oopery could no doubt represent such a breakdown and have
things like concatenate or compare.  That then work almost directly with the
bytes without lots of converting.
